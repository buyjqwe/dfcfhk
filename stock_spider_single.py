...
class stock_spider():
    ...
    def get_financial_data(self, secucode, page_number=1, page_size=9):
        ...
        for _ in range(retry_count):
            try:
                # 可以在此处添加 proxies=self.proxies 如果需要使用代理
...
if __name__ == '__main__':
    ...
    buffer_list = [] # 缓存列表
    save_interval = 10 # 每抓取10个保存一次，防止程序崩溃数据丢失

    print("开始单线程抓取 (为了防止被封，已启用延时)...")

    for id in range(start_stock_code_effective, end_stock_code):
        # 单线程直接调用
        result = s.fetch_one_stock(id)
        ...
        # === 关键点：防止被封的延时 ===
        # 建议设置 0.5 到 2 秒之间，如果还是被封，请调大这个数字
        sleep_time = random.uniform(0.5, 1.5) # 生成 1 到 3 之间的随机浮点数
